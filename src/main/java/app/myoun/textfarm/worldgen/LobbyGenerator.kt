...
typealias RecordCodecBuilderInstance<T> = RecordCodecBuilder.Instance<T>

class LobbyGenerator(biomeRegistry: RegistryEntryLookup<Biome?>) :
    ...
    companion object {
        val CODEC: MapCodec<LobbyGenerator?> =
            RecordCodecBuilder.mapCodec { instance: RecordCodecBuilderInstance<LobbyGenerator> ->
                return@mapCodec instance.group(RegistryOps.getEntryLookupCodec(RegistryKeys.BIOME))
                    .apply(
                        instance,
                        instance.stable(Function { biomeRegistry: RegistryEntryLookup<Biome?> ->
                            LobbyGenerator(biomeRegistry)
                        })
                    )
            }
    }
